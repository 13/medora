...
void main() {
  for (final (label, theme) in [
    ('light', AppTheme.lightThemeFrom(Colors.teal)),
    ('dark', AppTheme.darkThemeFrom(Colors.teal)),
  ]) {
    test('$label theme registers MedoraColors with readable pairs', () {
      final m = theme.extension<MedoraColors>();
      expect(m, isNotNull);
      for (final (fg, bg) in [
        (m!.onSuccess, m.success),
        (m.onSuccessContainer, m.successContainer),
        (m.onWarning, m.warning),
        (m.onWarningContainer, m.warningContainer),
        (m.onDanger, m.danger),
        (m.onDangerContainer, m.dangerContainer),
        (m.onNeutral, m.neutral),
        (m.onNeutralContainer, m.neutralContainer),
      ]) {
        expect(
          contrastRatio(fg, bg),
          greaterThanOrEqualTo(3.0),
          reason: '$label pair $fg on $bg',
        );
      }
    });
