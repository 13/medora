...
void main() {
  ...
  test(
    'saveFromPath copies into the photos folder and returns a bare filename',
    () async {
  ...
  test(
    'resolve maps a stale absolute path to the current folder by basename',
    () async {
      final name = await storage.saveFromPath(
        (File(p.join(root.path, 'x.jpg'))..writeAsBytesSync([1])).path,
      );
      final stale =
          '/var/mobile/Containers/OLD/Documents/${PhotoStorage.folder}/$name';
  ...
  test('delete and deleteAll remove files', () async {
    final a = await storage.saveFromPath(
      (File(p.join(root.path, 'a.jpg'))..writeAsBytesSync([1])).path,
    );
    final b = await storage.saveFromPath(
      (File(p.join(root.path, 'b.jpg'))..writeAsBytesSync([1])).path,
    );
